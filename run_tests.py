...
==========================================================
Runs all unit tests and reports pass/fail.

Fixes applied (exhaustive audit):
  - Removed unused import: traceback
  - Added numerical gradient verification tests for all 4 test functions
  - Added Beale tests for PR and HS variants
  - Added test: already-converged x0 (||grad_f(x0)|| < tol)
  - Added test: 2D x0 input is handled (ravel'd to 1D)
  - Added test: wolfe_line_search returns 4-tuple (alpha, f_new, nfev, ngev)
  - Updated GD history test to reflect post-update convention
  - Added test: fk_new from line search matches f(x + alpha*d) exactly
"""
import sys, os
...
import numpy as np
from ncg import (
    ncg_minimize, beta_fletcher_reeves, beta_polak_ribiere,
...
def t_fr_positive():
    g, go, d = _gradients()
    assert beta_fletcher_reeves(g, go, d) >= 0

def t_fr_formula():
    g, go, d = _gradients()
    exp = np.dot(g, g) / np.dot(go, go)
    assert abs(beta_fletcher_reeves(g, go, d) - exp) < 1e-14

def t_pr_can_be_negative():
    # g_new . y_k = [0.5]*[-1.5] = -0.75 < 0  ->  beta < 0
    g = np.array([0.5, 0.0]); go = np.array([2.0, 0.0]); d = np.zeros(2)
    assert beta_polak_ribiere(g, go, d) < 0

def t_prplus_nonneg():
    g = np.array([0.5, 0.0]); go = np.array([2.0, 0.0]); d = np.zeros(2)
    assert beta_polak_ribiere_plus(g, go, d) >= 0

def t_prplus_equals_pr_when_positive():
    g = np.array([1.5, 0.5]); go = np.array([1.0, 0.0]); d = np.zeros(2)
    assert abs(beta_polak_ribiere_plus(g, go, d)
               - max(beta_polak_ribiere(g, go, d), 0.0)) < 1e-14

def t_hs_zero_denom():
    # g_new == g_old  ->  y=0  ->  d.y=0  ->  return 0
    g = np.array([1.0, 0.0]); go = np.array([1.0, 0.0]); d = np.array([1.0, 0.0])
    assert beta_hestenes_stiefel(g, go, d) == 0.0

def t_hs_formula():
    g = np.array([1.2, 0.4]); go = np.array([0.8, 0.1]); d = np.array([0.3, -0.5])
    y = g - go
    denom = np.dot(d, y)
    if abs(denom) > 1e-30:
        exp = np.dot(g, y) / denom
        assert abs(beta_hestenes_stiefel(g, go, d) - exp) < 1e-12

def t_zero_old_grad_safeguard():
    # FR, PR, PR+ use ||g_old||^2 in denominator -> return 0 when g_old = 0
    go = np.zeros(5); g = np.ones(5); d = np.ones(5)
    for name in ["FR", "PR", "PR+"]:
        v = BETA_FUNCTIONS[name](g, go, d)
        assert v == 0.0, f"{name} should return 0 when g_old=0, got {v}"
    # HS uses d.y denominator; g_old=0 -> y=g, d.y=n != 0 -> safeguard N/A here

def t_fr_pr_equal_on_quadratic():
    # Exact case: g_new . g_old = 0 (mutually orthogonal residuals)
    g = np.array([0.0, 1.0]); go = np.array([1.0, 0.0]); d = np.zeros(2)
    fr = beta_fletcher_reeves(g, go, d)
    pr = beta_polak_ribiere(g, go, d)
    assert abs(fr - pr) < 1e-12, f"FR={fr}, PR={pr} should be equal"
...
def _check_grad(f, gf, x_tests, name, tol=1e-5):
    for x in x_tests:
        ana = gf(x)
        num = _numerical_grad(f, x)
        err = np.linalg.norm(ana - num)
        assert err < tol, f"{name}: gradient error {err:.2e} at x={x}"

def t_grad_rosenbrock():
    f, gf, _, _ = rosenbrock()
    _check_grad(f, gf, [np.array([-1.2, 1.0]), np.array([0.5, 0.25]),
                         np.array([1.0, 1.0])], "Rosenbrock")

def t_grad_beale():
    f, gf, _, _ = beale()
    _check_grad(f, gf, [np.array([1.0, 1.0]), np.array([2.0, 0.3]),
                         np.array([3.0, 0.5])], "Beale")

def t_grad_quadratic():
    f, gf, _, _ = quadratic_bowl_2d(kappa=20.0)
    _check_grad(f, gf, [np.array([3.0, 1.0]), np.array([0.0, 0.0]),
...
def t_grad_highdim():
    ...
    _check_grad(f, gf, [x_test], "HighDimQuadratic")
...
def t_wolfe_returns_4tuple():
    # Bug fix: signature changed to return (alpha, f_new, nfev, ngev)
...
def t_wolfe_fnew_matches_f():
    # Bug fix: f_new should equal f(x + alpha*d) exactly
    f, gf, x, fx, g, d = _rosen_setup()
    alpha, f_new, _, _ = wolfe_line_search(f, gf, x, d, fx, g)
    assert abs(f_new - f(x + alpha * d)) < 1e-14, \
        f"f_new={f_new} != f(x+alpha*d)={f(x + alpha*d)}"
...
def t_wolfe_non_descent_raises():
    f, gf, x, fx, g, d = _rosen_setup()
    try:
        wolfe_line_search(f, gf, x, -d, fx, g)   # ascent direction
        assert False, "Should have raised AssertionError"
    except AssertionError:
        pass

def t_wolfe_on_beale():
    # Verify Wolfe conditions also hold on Beale's function
    f, gf, xs, _ = beale()
    x = np.array([1.0, 1.0]); g = gf(x); d = -g
    alpha, f_new, _, _ = wolfe_line_search(f, gf, x, d, f(x), g, c1=1e-4, c2=0.9)
    dphi0 = np.dot(g, d)
    assert f_new <= f(x) + 1e-4 * alpha * dphi0 + 1e-10  # Armijo
    assert abs(np.dot(gf(x + alpha * d), d)) <= 0.9 * abs(dphi0) + 1e-8  # Curvature
...
def _test_conv(f, gf, x0, xs, variant, max_iter=3000, grad_atol=None, x_atol=1e-3):
    ...
    assert res.grad_norm < grad_atol, \
        f"{variant}: ||grad_f||={res.grad_norm:.2e} >= {grad_atol:.2e}"
    err = np.linalg.norm(res.x - xs)
    assert err < x_atol, f"{variant}: ||x-x*||={err:.2e} >= {x_atol:.2e}"
...
def t_finite_termination():
    # In exact arithmetic, NCG on an n-D quadratic terminates in <= n steps.
    # With floating-point Wolfe line search, we verify convergence only.
    n = 10
    f, g, xs, _ = high_dim_quadratic(n=n, kappa=5, seed=2)
...
def t_already_converged_x0():
    # If x0 is already the minimizer, solver should return immediately (0 iterations)
...
def t_2d_x0_raveled():
    # x0 passed as 2D array should be silently ravel()ed to 1D
    f, gf, xs, _ = quadratic_bowl_2d()
...
def t_periodic_restart():
    f, g, _, _ = rosenbrock()
    res = ncg_minimize(f, g, np.array([-1.2, 1.]), beta_variant="PR+",
                       restart_every=5, use_powell_restart=False,
                       tol=1e-8, max_iter=200)
    # restart_every=5 means restart fires when (k+1) % 5 == 0, i.e. at k=4 (index 4)
    assert res.restart_flags[4], "Periodic restart should fire at completed step 5 (index 4)"

def t_powell_restart_fires():
    f, g, _, _ = rosenbrock()
    res = ncg_minimize(f, g, np.array([-1.2, 1.]), beta_variant="FR",
...
def t_powell_independent_of_periodic():
    # Powell should be able to fire on a periodic-restart iteration too.
    # With the old elif design this was impossible; with the fix both fire.
    f, g, _, _ = rosenbrock()
    res = ncg_minimize(f, g, np.array([-1.2, 1.]), beta_variant="FR",
                       restart_every=2, use_powell_restart=True, powell_nu=0.0,  # nu=0 -> always
                       tol=1e-8, max_iter=50)
    # nu=0 means Powell always fires; periodic also fires at k=1,3,5...
    # Combined: restart_flags[k] should be True for ALL k (both criteria fire)
    assert all(res.restart_flags), "Both periodic and Powell should fire every step"


# ===========================================================================
# 6. Result structure and counting
# ===========================================================================
...
def t_no_history_when_disabled():
    f, g, _, _ = quadratic_bowl_2d()
    res = ncg_minimize(f, g, np.array([3., 3.]), tol=1e-6, max_iter=100,
                       store_history=False)
...
def t_invalid_variant_raises():
    f, g, _, _ = quadratic_bowl_2d()
    try:
        ncg_minimize(f, g, np.zeros(2), beta_variant="INVALID")
...
def t_x_shape_preserved():
    ...
    res = ncg_minimize(f, g, np.zeros(20), tol=1e-6, max_iter=200)
    assert res.x.shape == (20,)

def t_nfev_count_accurate():
    # Verify reported nfev matches actual calls to f
    actual = [0]
    f_r, gf_r, _, _ = rosenbrock()
    def f_counted(x): actual[0] += 1; return f_r(x)
    def g_counted(x): return gf_r(x)
    res = ncg_minimize(f_counted, g_counted, np.array([-1.2, 1.0]),
...
def t_fk_no_redundant_eval():
    # Bug fix: fk_new is returned by line search so no extra f-eval.
    # Total nfev should be 1 (initial) + sum of line search evals (no +nit extra).
    actual_f = [0]; actual_g = [0]
    f_r, gf_r, _, _ = quadratic_bowl_2d()
    def fc(x): actual_f[0] += 1; return f_r(x)
    def gc(x): actual_g[0] += 1; return gf_r(x)
    res = ncg_minimize(fc, gc, np.array([4., 4.]), beta_variant="PR+",
                       tol=1e-8, max_iter=20)
    # With fix: nfev should equal actual_f[0] exactly
