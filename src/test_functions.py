...
========================================================
Implements all four test functions specified in the checklist:
  1. Quadratic bowl (2-D)
  2. Rosenbrock function
  3. Beale's function
  4. High-dimensional quadratic

Each function is provided as a pair  (f, grad_f) with known
global minimizer and minimum value for verification.
"""
...
def quadratic_bowl_2d(kappa: float = 50.0
    ...
    Condition number κ chosen large (default 50) to expose the
    zig-zagging behaviour of gradient descent vs. NCG (cf. Figure 1
    in the report).
...
def high_dim_quadratic(n: int = 100, kappa: float = 100.0,
    ...
    The matrix A is constructed with eigenvalues log-spaced between 1 and
    κ so that cond(A) = κ. This is the canonical setting for proving NCG
    terminates in n steps in exact arithmetic (Section 6 of the report).
    ...
    # Build A with eigenvalues log-spaced in [1, kappa]
    eigvals = np.logspace(0, np.log10(kappa), n)     # shape (n,)
    Q, _ = np.linalg.qr(rng.standard_normal((n, n))) # random orthogonal Q
...
def get_all_test_problems():
    """
    Returns a list of dicts, each describing one test problem.
    ...
    problems.append(dict(name="Quadratic Bowl 2D (κ=50)",
                         f=f, grad_f=g,
                         x0=np.array([4.0, 4.0]),
                         x_star=xs, f_star=fs, dim=2))

    # --- Rosenbrock ----------------------------------------------------------
    f, g, xs, fs = rosenbrock()
    problems.append(dict(name="Rosenbrock",
                         f=f, grad_f=g,
                         x0=np.array([-1.2, 1.0]),
                         x_star=xs, f_star=fs, dim=2))
    ...
    # --- Beale ---------------------------------------------------------------
    f, g, xs, fs = beale()
    problems.append(dict(name="Beale",
                         f=f, grad_f=g,
                         x0=np.array([1.0, 1.0]),
                         x_star=xs, f_star=fs, dim=2))
    ...
    problems.append(dict(name="High-Dim Quadratic (n=100, κ=100)",
                         f=f, grad_f=g,
                         x0=np.zeros(100),
                         x_star=xs, f_star=fs, dim=100))
